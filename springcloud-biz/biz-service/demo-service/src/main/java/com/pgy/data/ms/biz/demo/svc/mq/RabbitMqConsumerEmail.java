package com.pgy.data.ms.biz.demo.svc.mq;
...
public class RabbitMqConsumerEmail {
     ...
     */
    @Bean
    public MessageListenerContainer emailMessageListenerContainer(ConnectionFactory connectionFactory) {
        SimpleMessageListenerContainer container = new SimpleMessageListenerContainer();
        container.setConnectionFactory(connectionFactory);
        container.setQueueNames(RabbitMqEnum.QueueName.EMAILNOTIFY.getCode());
        //设置监听信道
        container.setMessageListener(emailChannelAwareMessageListener());
        //设置确认模式
     ...
     */
    @Bean
    public ChannelAwareMessageListener emailChannelAwareMessageListener() {
        return (message, channel) -> {
            try {
                byte[] body = message.getBody();
                String string = new String(body);
                log.info("获取到" + RabbitMqEnum.QueueName.EMAILNOTIFY.getCode() + "队列中的消息：{}", string);
            } catch (Exception e) {
                //发声错误再把消息发送到队列
                e.printStackTrace();
            } finally {
