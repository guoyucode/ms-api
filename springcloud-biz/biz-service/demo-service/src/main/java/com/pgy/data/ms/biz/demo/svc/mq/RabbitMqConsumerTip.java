...
public class RabbitMqConsumerTip {
     ...
     */
    @Bean
    public MessageListenerContainer tipMessageListenerContainer(ConnectionFactory connectionFactory) {
    ...
    public ChannelAwareMessageListener tipChannelAwareMessageListener() {
        return (message, channel) -> {
            try {
                ...
                log.info("获取到" + RabbitMqEnum.QueueName.SYSTEMNOTIFY.getCode() + "队列中的消息：{}", string);
            } catch (Exception e) {
                //发声错误再把消息发送到队列
                e.printStackTrace();
            } finally {
